                             ...
                             uint8_t *buf, uint16_t ps, int32_t *off)
{
    ...
    energest_flush();
    {
        unsigned long ct = energest_type_time(ENERGEST_TYPE_CPU);
        unsigned long lt = energest_type_time(ENERGEST_TYPE_LPM);
        unsigned long tt = energest_type_time(ENERGEST_TYPE_TRANSMIT);
        unsigned long rt = energest_type_time(ENERGEST_TYPE_LISTEN);
        cpu_auth_as    = ct / (double)ENERGEST_SECOND;
        double sl = lt / (double)ENERGEST_SECOND;
        double st = tt / (double)ENERGEST_SECOND;
        double sr = rt / (double)ENERGEST_SECOND;
        energy_auth_as = cpu_auth_as*1.8e-3*3.0 + sl*0.0545e-3*3.0
                       + st*17.4e-3*3.0 + sr*18.8e-3*3.0;
